...
describe Round do
  let(:human) { double(:human, piece: 'X', next_move: 5, :piece= => nil) }
  let(:ai) { double(:ai, piece: 'O', next_move: 5, :piece= => nil) }
  let(:game) { double(:game, winner: nil, draw?: nil, over?: nil, make_move: nil, board_string: '---------', empty_space: '-', valid_move?: true) }
  let(:view) { double(:view, show_start: nil, show_turn: nil, show_invalid_move: nil, show_draw: nil, show_winner: nil, print_board: nil, show_record: nil, ask_for_piece: 'O', ask_for_order: 0) }
  ...
  describe 'starting the round' do
    it 'asks the player for their choice of piece' do
      expect(view).to receive(:ask_for_piece)
      round.stub(:start)
      round.setup
    end

    it 'asks the player for their choice of order' do
      expect(view).to receive(:ask_for_order)
      view.stub(:ask_for_order).and_return(0)
      round.stub(:start)
      round.setup
    end
